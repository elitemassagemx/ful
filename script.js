...
const UIModule = {
    ...
    setupDailyUINav: () => {
        ...
        document.addEventListener('click', (e) => {
            if (!(e.target instanceof HTMLElement) || !e.target.closest('.dropdown-wrapper')) {
...
const ExperiencesModule = {
        ...
        switches.forEach(switchInput => {
            switchInput.addEventListener('change', (e) => {
                ...
                if (target instanceof HTMLInputElement) {
                    const switchLabel = target.nextElementSibling?.querySelector('.switch__text');
                    if (switchLabel instanceof HTMLElement) {
                        switchLabel.textContent = target.checked ? 'Activado' : 'Desactivado';
                    }
                }
            });
        });
    ...
    setupExperienciasCards: () => {
        const experienciasContainer = document.querySelector('.cols');
        if (experienciasContainer) {
            experienciasContainer.addEventListener('click', (e) => {
                const target = e.target;
                if (target instanceof HTMLElement) {
    ...
    init: () => {
        ...
        ExperiencesModule.setupExperienciasCards();
    }
};
